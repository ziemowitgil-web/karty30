...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <a href="{{ route('consultations.create') }}" class="bg-green-800 text-white px-4 py-2 rounded hover:bg-blue-700 focus:outline-none focus:ring-3 focus:ring-offset-2 focus:ring-blue-400 mb-6 inline-block">
                ...
                @foreach($consultations->where('status','draft') as $c)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-2 flex flex-wrap gap-2">
                            <button class="sign-button bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-400" data-id="{{ $c->id }}" aria-label="Podpisz konsultację {{ $c->id }}">Podpisz</button>
                            <button class="history-button bg-gray-500 text-white px-3 py-1 rounded hover:bg-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400" data-id="{{ $c->id }}" aria-label="Historia konsultacji {{ $c->id }}">Historia</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                @foreach($consultations->where('status','completed') as $c)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-2 flex flex-wrap gap-2">
                            <button class="history-button bg-gray-500 text-white px-3 py-1 rounded hover:bg-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400" data-id="{{ $c->id }}" aria-label="Historia konsultacji {{ $c->id }}">Historia</button>
                            <a href="{{ route('consultations.pdf', $c) }}" target="_blank" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-400" aria-label="Drukuj konsultację {{ $c->id }}">Drukuj</a>
                            <a href="{{ route('consultations.xml', $c) }}" target="_blank" class="bg-purple-500 text-white px-3 py-1 rounded hover:bg-purple-600 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-purple-400" aria-label="Podgląd XML konsultacji {{ $c->id }}">Podgląd XML</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    <div id="signModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50" role="dialog" aria-modal="true" aria-labelledby="signModalTitle">
        <div class="bg-white p-6 rounded-xl max-w-2xl w-full shadow-lg relative">
            <button id="closeSignModal" class="absolute top-4 right-4 bg-gray-200 px-3 py-1 rounded hover:bg-gray-300 focus:outline-none" aria-label="Zamknij modal podpisu">Zamknij</button>
            <h3 id="signModalTitle" class="text-2xl font-bold mb-4 text-center">Proces podpisywania karty konsultacji</h3>
            <p class="text-gray-600 mb-6 text-center">Podpis cyfrowy zapewnia bezpieczeństwo i integralność dokumentu.</p>
            ...
            <ol id="signSteps" class="space-y-4">
                @foreach([
                    ...
                    <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow">
    ...
    <div id="historyModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50" role="dialog" aria-modal="true" aria-labelledby="historyModalTitle">
        <div class="bg-white p-6 rounded-lg max-w-lg w-full">
            ...
            <button id="closeHistoryModal" class="mt-4 bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400">Zamknij</button>
        </div>
    </div>
                    ...
                    function nextStep() {
                        if(current < steps.length){
                            ...
                            fetch(`/consultations/${consultationId}/sign-json`, {
                                ...
                                .then(data=>{
                                    if(data.success){
                                        document.getElementById('shaDisplay').textContent = data.sha1 || 'GENEROWANY...';
                                        alert(data.message);
                                        location.reload();
                                    } else {
