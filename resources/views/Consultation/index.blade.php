...
@section('content')
        ...
        <div class="overflow-x-auto shadow rounded-lg border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Data i godzina</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Czas trwania</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Odcisk palca (fingerprint)</th>
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Akcje</th>
                </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
    ...
    <div id="signModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-xl max-w-2xl w-full shadow-lg relative">
            <!-- Zamknij -->
            <button id="closeSignModal" class="absolute top-4 right-4 text-gray-500 hover:text-gray-800">
                &times;
            </button>
            ...
            <p class="text-gray-600 mb-6 text-center">
                Podpis cyfrowy zapewnia bezpieczeństwo i integralność dokumentu.
                Każdy etap jest monitorowany, a dane są szyfrowane i oznaczane unikalnym SHA1.
            </p>
            ...
            <ol id="signSteps" class="space-y-4">
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">1</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Weryfikacja uprawnień systemu</span>
                        <span class="text-gray-400 text-sm">Sprawdzenie uprawnień i  serwera podpisu.</span>
                    </div>
                </li>
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">2</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Pobranie certyfikatu użytkownika</span>
                        <span class="text-gray-400 text-sm">Certyfikat  pobrany dla  {{ Auth::user()->name }}.</span>
                    </div>
                </li>
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">3</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Weryfikacja integralności dokumentu XML</span>
                        <span class="text-gray-400 text-sm">Sprawdzanie poprawności struktury i danych.</span>
                    </div>
                </li>
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">4</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Proces zatwieerdzania dokumentu w formie dokumentowej (pdopis FEER)</span>
                        <span class="text-gray-400 text-sm">Dokument zostaje podpisany za pomocą algorytmów .</span>
                    </div>
                </li>
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">5</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Szyfrowanie danych i generowanie SHA1</span>
                        <span class="text-gray-400 text-sm">SHA1 zostaje wygenerowane i zapisane dla dokumentu.</span>
                    </div>
                </li>
                <li class="step flex items-center gap-4 p-4 border rounded-lg bg-white shadow transition duration-300 hover:scale-105">
                    <div class="step-number w-12 h-12 flex items-center justify-center rounded-full border-2 border-gray-300 text-gray-500 font-bold text-lg">6</div>
                    <div class="flex flex-col">
                        <span class="text-gray-700 font-medium">Czyszczenie danych tymczasowych i finalizacja</span>
                        <span class="text-gray-400 text-sm">Pliki tymczasowe są usuwane, proces kończy się sukcesem.</span>
                    </div>
                </li>
            </ol>
            ...
            <div class="mt-8 p-4 bg-gray-100 rounded-lg shadow text-center font-mono text-gray-800 text-lg">
                SHA1 dokumentu: <span id="shaDisplay">{{ $consultation->sha1sum ?? 'GENEROWANY...' }}</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Animacja podświetlania kroków i postępu
    ...
    <script>
        document.querySelectorAll('.sign-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const steps = document.getElementById('signSteps').children;
                let current = 0;

                function nextStep() {
                    if(current < steps.length){
                        steps[current].classList.add('text-green-600','font-semibold');
                        setTimeout(()=>{
                            current++;
                            nextStep();
                        }, 700);
                    } else {
                        fetch(`/consultations/${consultationId}/sign`, {
                            method: 'POST',
                            headers: {
                            ...
                            .then(data => {
                                ...
                                alert(data.message || 'Proces zakończony');
                                location.reload();
                            })
                            .catch(err => {
                                modal.classList.add('hidden');
