    ...
    <div class="container mx-auto px-6 py-8 space-y-6" role="main" aria-label="Dashboard użytkownika">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <main class="lg:col-span-3 space-y-6" role="region" aria-label="Panel główny">
                ...
                @php
                    $certFile = storage_path('app/certificates/' . auth()->user()->id . '_user_cert.pem');
                ...
                <section class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 space-y-2" aria-label="Informacje o certyfikacie użytkownika">
                    ...
                    @if($certCN !== 'Brak certyfikatu')
                        <div class="px-3 py-2 rounded bg-yellow-500 text-gray-900 font-semibold text-sm truncate" title="{{ $certCN }}">
                            CN: {{ $certCN }}
                        </div>
                        <p class="text-gray-500 text-xs mt-1">Posiadanie certyfikatu jest wymagane do podpisywania dokumentów.</p>
                    @else
                        <p class="text-red-600 font-semibold text-sm">Brak certyfikatu — nie możesz podpisywać dokumentów.</p>
                    @endif
                </section>
