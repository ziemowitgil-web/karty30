...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        {{-- ALERT REDIS / SYSTEM KOLEJKOWANIA --}}
        @if($redisStatus !== 'Dostępny')
            <div id="redis-alert" class="mb-4 p-4 bg-red-100 border border-red-300 text-red-800 rounded-lg relative flex justify-between items-center">
                <span>{{ $redisStatus }}</span>
                <button onclick="document.getElementById('redis-alert').style.display='none'" class="ml-4 text-red-600 hover:text-red-800 font-bold text-lg">&times;</button>
            </div>
        @endif
        ...
        @foreach (['warning','error','success'] as $msg)
            @if(session($msg))
                ...
                @endphp
                <div class="mb-4 p-4 bg-{{ $colors[$msg] }}-100 border border-{{ $colors[$msg] }}-300 text-{{ $colors[$msg] }}-800 rounded-lg">
                    {{ session($msg) }}
                </div>
            @endif
        @endforeach
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {{-- LEWY PANEL --}}
            <aside class="lg:col-span-1 bg-white border border-gray-200 rounded-2xl shadow-sm p-4 space-y-6">
                <header class="mb-4">
                    <h2 class="text-lg font-semibold text-gray-800">Witaj, {{ auth()->user()->name }}</h2>
                    <p class="text-xs text-gray-500 mt-1">Twoje podsumowanie dnia i ostatnie aktywności.</p>
                </header>
                ...
                <section class="space-y-2">
                    ...
                    <div class="p-2 bg-yellow-50 border border-yellow-100 rounded-lg flex justify-between items-center">
                        ...
                        <i class="fas fa-pencil-alt text-yellow-700"></i>
                    </div>
                ...
                <section>
                    ...
                    @if($recentActions->isEmpty())
                        <p class="text-gray-500 text-xs">Brak ostatnich akcji.</p>
                    @else
            ...
            <main class="lg:col-span-3 space-y-6">
                ...
                @if(config('app.test_mode') != 1)
                    <section class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
                        ...
                        <p class="text-gray-500 text-xs mb-2">
                            Możesz użyć <strong>klucza bezpieczeństwa (WebAuthn)</strong> lub <strong>certyfikatu PFX</strong> do logowania oraz podpisywania dokumentów.
                        </p>
                        <button id="register-key-btn" class="w-full mb-1 bg-gray-800 hover:bg-gray-900 text-white py-1.5 rounded-md transition-colors text-sm">
                            Zarejestruj klucz
                        </button>
                        <a href="#" class="w-full inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white py-1.5 rounded-md transition-colors text-sm">
                            Użyj certyfikatu PFX
                        </a>
                    </section>
                @else
                ...
                <section class="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
                    ...
                    @if($todaySchedules->isEmpty())
                        <p class="text-gray-500 text-xs">Brak zaplanowanych rezerwacji.</p>
                    @else
